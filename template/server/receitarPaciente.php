<?php
    ...
    foreach($lista_consultas as $consulta) {
        $receita = $_POST["receita$cont"];
        
        if($receita != "")
            inserirReceita($receita, $consulta);
        
        $cont = $cont + 1;
    }

    function inserirReceita($receita_nova, $consulta_para_buscar) {
        ...
        foreach($root->children() as $consulta) {
            if(verificarIgualdadeConsultas($consulta, $consulta_para_buscar)){
                $consulta->receita = $receita_nova;
                echo "Receita salva com sucesso.<br>";
            }
